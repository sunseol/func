...
| QA-DOC-003 | version/history API | Given a document edited twice, When the versions API is read by an authorized admin, Then versions 1/2/3 contain the exact initial and edited content for that document; an outsider receives 404 `DOCUMENT_NOT_FOUND` because inaccessible documents are hidden | seed doc and two API edits; API request contexts | versions GET; exact version/document/content tuples; outsider 404 `DOCUMENT_NOT_FOUND`; DB read otherwise | delete project/docs | Playwright `ai-pm-access-control.spec.ts` | live-integration | covered |
...
| QA-DOC-005 | request/approve/withdraw API transitions | Given private documents and authorized/unauthorized project roles, When request, approve, or withdraw is called, Then the implemented request/approve/withdraw endpoints update status as supported, request/approve history is exact, unauthorized approval receives 404 `DOCUMENT_NOT_FOUND` because inaccessible documents are hidden, and repeated transitions return bounded 400 errors; reject endpoint and viewport/UI claims are not part of this API-only contract | role matrix; API request contexts | endpoint 2xx/404/400; unauthorized approval returns `DOCUMENT_NOT_FOUND`; DB status/history exact for request and approve; no reject route is claimed | delete project/docs | Playwright `ai-pm-access-control.spec.ts` | live-integration | covered |
