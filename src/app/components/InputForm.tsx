...
interface InputFormProps {
  onDataChange: (data: {
    userName: string;
    date: string; // 부모 컴포넌트에는 string으로 전달
    projects: Project[];
    miscTasks: TaskItem[];
  }) => void;
...
export default function InputForm({ onDataChange, initialData }: InputFormProps) {
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      // 날짜 문자열을 Dayjs 객체로 변환, 유효하지 않으면 null
  ...
  return (
    ...
    >
      <Card title="기본 정보" style={{ marginBottom: 24 }}>
        ...
        >
          <DatePicker style={{ width: '100%' }} format="YYYY.MM.DD(ddd)" />
        </Form.Item>
      </Card>
                      ...
                      <Space direction="vertical" style={{ width: '100%' }}>
                        {taskFields.map(({ key: taskKey, name: taskName, ...restTaskField }) => (
                          <Space key={taskKey} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                            ...
                            >
                              <Input.TextArea placeholder="업무 내용" autoSize={{ minRows: 1, maxRows: 3 }} />
                            </Form.Item>
                             <Form.Item
                              {...restTaskField}
              ...
              {fields.map(({ key, name, ...restField }) => (
                <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                  <Form.Item
                    ...
                    style={{ flexGrow: 1, marginBottom: 0 }}
                  >
                    <Input.TextArea placeholder="기타 업무 내용" autoSize={{ minRows: 1, maxRows: 3 }} />
                  </Form.Item>
                  <Form.Item
                    {...restField}
